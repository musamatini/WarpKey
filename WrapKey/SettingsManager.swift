...
private let cheatsheetShortcutKey = "WrapKey_CheatsheetShortcut_v1"
...
@Published var cheatsheetShortcut: [ShortcutKey] {
    didSet { saveCheatsheetShortcut() }
}
...
init() {
    ...
    if let data = UserDefaults.standard.data(forKey: cheatsheetShortcutKey),
       let decodedShortcut = try? JSONDecoder().decode([ShortcutKey].self, from: data) {
        self.cheatsheetShortcut = decodedShortcut
    } else {
        self.cheatsheetShortcut = []
    }
...
private func saveCheatsheetShortcut() {
    if let encoded = try? JSONEncoder().encode(cheatsheetShortcut) {
        UserDefaults.standard.set(encoded, forKey: cheatsheetShortcutKey)
    }
}
